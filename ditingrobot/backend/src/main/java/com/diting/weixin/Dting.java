...
public class Dting {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(Dting.class);
...
//	//微信机器人的 url和微信名称
		...
		if(StringKit.isNotBlank(res)){
			...
			if(null != code){
				if(code.equals("200")){
					...
					LOGGER.info("错误的状态码: %Matchers.javas", code);
				}
			}
		}
		return null;
	 ...
	 * 显示二维码
	 */
	public void showQrCode(String urlname) {
			...
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						...
						qrCodeFrame = new QRCodeFrame(output.getPath(),urlname);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
...
//			}
				...
				LOGGER.info(Thread.currentThread().getName() +"  扫描code=  ", code);
			}
		}
		return code;
	}
	...
	public void handleMsg(JSONObject data){
		...
		for(int i=0,len=AddMsgList.size(); i<len; i++){
			...
			String content = msg.getString("Content").replace(" ","");
			...
			if(msgType == 51){
				...
				if(SpecialUsers.contains(msg.getString("ToUserName"))){
					...
					continue;
				}else if(content.contains("@") && content.contains("<br/>")){
					//原始字符串   content
					String matter1 = "@";//目标字符串1
					String matter2 = "<br/>";//目标字符串2
					int a = content.indexOf(matter1);//第一个字符串的起始位置
					int b = content.indexOf(matter2);//第二个字符串的起始位置
					String result = content.substring(0, a+matter1.length())+content.substring(b, content.length());//利用substring进行字符串截取
					String que = result.replace("<br/>","").substring(1).replace(" ", "");

					if(que.contains(Dting.name)){
						String ress = que.replace(Dting.name,"");

						String ans = xiaodoubi(ress);
				...
				if(content.contains("收到红包")){//收到红包

				}else if(content.contains("加入了群聊")){//加入了群聊

				}else if(content.contains("移出了群聊")){

				}else if(content.contains("分享的二维码加入群聊")){
    ...
    */
	private String xiaodoubi(String msg) {
		...
		try {
	        ...
	        json.put("question",msg);
	        json.put("username",url);
	 
	    	String s = HttpUtils.sendRequestWithHttpURLConnection("http://www.ditingai.com/remote/chat/info",json.toString());
			com.alibaba.fastjson.JSONObject js = com.alibaba.fastjson.JSON.parseObject(s);
			String res = js.getString("answer");
			return res;
		} catch (Exception e) {
			return "请求失败";
		}
...
//								System.out.println("arr[1] == 2 这里发生错误了 ");
								continue;
							}
							
						} else if(arr[1] == 6){ //获取最新消息
							try {
								JSONObject data = webwxsync();
								handleMsg(data);
							} catch (Exception e) {
//								System.out.println("arr[1] == 6 这里发生错误了 ");
								continue;
							}
...
//								System.out.println("arr[1] == 7 这里发生错误了 ");
								continue;
							}	
						} else if(arr[1] == 3){
							try {
...
//								System.out.println("arr[1] == 3 这里发生错误了 ");
								continue;
							}
						} else if(arr[1] == 0){
							try {
	 ...
	 *
	 * 程序启动的入口 启动后会弹出二维码
	 * 用手机微信扫描二维码 可以接入到自动回复
	 *
     */
//	public static void main(String[] args) throws InterruptedException {
		public  void ma(String url ,String name) throws InterruptedException {
