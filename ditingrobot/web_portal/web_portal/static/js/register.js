...
function NewYear(){
    var account = {
        userName:$("#phone").val(),
        robotName:$('#name').val(),
        newYearGreetings:$('#newYearGreetings input[name="Greetings"]:checked ').val(),
        mobile: $("#phone").val(),
   ...
   console.log(account)
    $.ajax({
        ...
        layer.msg(data.message, {time:800,icon: 6}, function () {
            window.location.href = "https://sojump.com/jq/12118234.aspx"
            // $('#regsBody').css("overflow","hidden")
            // $('#regsBody').on('touchmove touchstart', function (event) {
            ...
            //     area: ['6.4rem', '10rem'],
            //     content: 'https://sojump.com/jq/12027444.aspx',
            //     cancel: function(index){
            //         window.location.href = "/m/cornerMatching"
            //     }
            // });
        })
    }).fail(function (data) {
